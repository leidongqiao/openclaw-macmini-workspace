...
4. **垂直源失败处理**：机器之心、量子位、36氪等垂直源经常 403、JS 渲染失败或只返回页脚/广告/数据服务页。遇到这种情况按“该源无有效正文”记录并跳过，不要反复重试；用新华网、财联社、人民网/杭州网、iFinD、精准搜索补足。
...
6. **iFinD 工具权限失败**：`search_trending_news` 可能返回 `Tool not allowed`。这不是致命错误：记录失败，不超过 1 次重试；继续执行 `search_news`、`search_notice` 和 web 搜索补足，不要中断周报。
7. **飞书 Drive 上传路径**：`lark-cli drive +upload --file` 不接受工作区外的绝对路径。上传前先 `cd` 到文件所在目录，再用 `--file ./文件名.docx` 相对路径。
8. **飞书 Drive 下载链接**：上传结果若只返回 `file_token`，需拼出完整 Drive 链接：`https://qcn8k445rrbc.feishu.cn/file/<file_token>`；wiki 正文和最终摘要都使用该完整链接，不要只写 token 或本地路径。
9. **wiki 创建位置**：`wiki +node-create --as bot --space-id` 仍可能把节点建到「首页」下。创建后必须用 `wiki nodes list` 或返回值检查 `parent_node_token`；若不为空，立即 `wiki +move --node-token ... --target-space-id ... --target-parent-token ''` 移回根目录。
10. **wiki 抽查必须解析 JSON 后检查**：`docs +fetch` 输出是 JSON；检查字段黏连时必须先解析 `data.markdown`，不要在原始 stdout 字符串上做正则，否则 `\n` 转义会导致误判。
...
- 文件名格式：`行业商机周报_【目标行业】_YYYYMMDD.docx`
- 字体使用华文楷体
...
6. **知识库标题**：AI-weekly-YYYYMMDD
7. **知识库写入**：必须使用 `lark-cli wiki +node-create --as bot --space-id 7630717889183534041` 创建，禁止使用 `feishu_wiki_space_node` 工具。去重时**搜索全部节点**（不限 parent_node_token），避免重复创建
8. **群聊推送**：推送概要 + 链接，不是全文
...
12. **Word 输出**：使用华文楷体字体，固定保存到 `/Users/leidongqiao/.openclaw/workspace/workspace-AIResearcher/reports/ai-weekly/`，文件名格式 `行业商机周报_【目标行业】_YYYYMMDD.docx`；生成后上传飞书 Drive，并将下载链接写在 wiki 正文开头及推送摘要中；Word 正文必须清理 Markdown 标记，推荐产品组合不用表格。
