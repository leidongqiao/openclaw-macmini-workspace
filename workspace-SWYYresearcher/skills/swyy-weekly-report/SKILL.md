...
- **web_search 优先**：用关键词精准搜索生物医药行业信息，比爬取大站再筛选高效得多
...
```
1. "创新药" OR "新药获批" OR "IND获批" OR "临床试验" OR "CDE审评" OR "NDA" → web_search（近7天，count=10）
2. "CXO" OR "CRO" OR "CDMO" OR "药物研发" OR "CRO融资" OR "CDMO订单" → web_search（近7天，count=10）
3. "医疗器械" OR "IVD" OR "高值耗材" OR "影像设备" OR "手术机器人" OR "三类器械获批" → web_search（近7天，count=10）
4. "集采" OR "医保谈判" OR "医保目录" OR "带量采购" OR "DRG/DIP" → web_search（近7天，count=10）
5. "生物医药融资" OR "生物药上市" OR "细胞治疗" OR "基因治疗" OR "ADC" OR "双抗" → web_search（近7天，count=10）
```
...
8. 米内网/中国药店 → web_fetch https://www.menet.com.cn/ (extractMode="text", maxChars=6000)
9. 丁香园/Insight数据库 → web_fetch https://www.dxy.cn/ (extractMode="text", maxChars=6000)
10. 药智网 → web_fetch https://www.yaozh.com/ (extractMode="text", maxChars=6000)
```

**第四轮：本地生物医药专项搜索（1个）：**

```
11. "生物医药 [地域名]" OR "创新药 [核心城市]" OR "医疗器械 [另一城市]" OR "CXO [地域名]" → web_search（近7天，count=10）
```
...
**入选标准（满足任一即可）：**
- 政策发布/修订（医保/集采/CDE审评指导原则/生物医药产业扶持政策，影响资金流向或授信风险）
- 企业融资/上市/定增公告（创新药企IPO、CXO融资、医疗器械企业定增等）
- 产能建设/项目开工/产线投产/新园区奠基（CDMO基地、GMP车间、生物药产业化基地等）
- 订单/中标/框架协议公告（集采中标、医院采购、CXO大订单等）
- 技术突破/产品获批（新药NDA/IND获批、临床试验结果公布、三类医疗器械注册证、新靶点/新疗法等）
- 价格变动（集采降价幅度、医保谈判价格、原料药价格波动，影响产业链成本和利润）
- 风险信号（临床试验失败、FDA/CDE不予批准、集采丢标、核心品种被调出医保、专利到期/侵权诉讼等）

**排除标准：**
- 行业常识科普
- 长期趋势分析（无具体事件）
- 与生物医药行业（创新药、CXO、医疗器械、疫苗、IVD、生物制品等）无关的内容
- 纯股市行情分析（无产业事件支撑）
...
**重要性评级：**
- ★★★★★：国家级政策（医保目录调整/集采新规/CDE重磅指导原则）/ 百亿级投资 / 行业拐点事件
...
**筛选标准（按权重排序）：**
1. 国家级政策（医保/集采/CDE）/百亿级投资（★★★★★）
2. 省级政策/十亿级融资/重大集采中标/重磅新药获批（★★★★）
3. 产能变动（CDMO扩建/GMP投产）/集采价格变动/重大临床进展（★★★）
...
```
企业 | 事件 | 资金需求/业务机会 | 优先级
XX生物制药（<城市>）| IPO受理 | 募资专户 | P0
XX医疗器械（<城市>）| 三类器械获批 | 产能扩建融资 | P1
XX CXO（<城市>）| 获得大订单 | 订单融资/保理 | P1
```
...
产品维度：
- 募集资金监管：X家企业，预计X亿
- 项目融资/固定资产贷款（CDMO基地/GMP车间/研发中心）：X家企业，预计X亿
- 研发贷/科技信贷（创新药研发/临床试验）：X家企业，预计X亿
- 订单融资/保理（集采中标/CXO大订单）：X家企业，预计X亿
- 流贷追加：X家企业，预计X亿
- 专精特新贷/高新技术企业贷：X家企业，预计X亿
- 供应链金融（原料药/医药流通）：X家企业，预计X亿
- 汇率锁汇（出口型CXO/医疗器械企业）：X家企业，预计X亿
```
...
SPREADSHEET_TOKEN = "AQjXsU9DehPYPEtDlZbcYY6Wnsf"
SHEET_ID = "cfde3f"
APP_ID = "cli_a97152da78799bc9"
APP_SECRET = subprocess.run(
    ['bash', '-c', 'jq -r \'.channels.feishu.accounts.swyy_bot.appSecret\' ~/.openclaw/openclaw.json'],
...
```
📌 生物医药行业周报·<地域> | YYYY.MM.DD-MM.DD（近7天）
...
[杭州]
- XX生物制药获X亿融资用于ADC产业化（X月X日）
- XX医疗器械三类注册证获批（X月X日）

[宁波]
- XX CXO签署X亿元海外大订单（X月X日）
...
【XX生物制药】（杭州）
事件：IPO受理 | 机会：募资专户 | 优先级：P0

【XX医疗器械】（宁波）
事件：三类器械获批 | 机会：产能扩建融资 | 优先级：P1

【XX CXO】（绍兴）
事件：获海外大订单 | 机会：订单融资/保理 | 优先级：P1
...
- 募集资金监管：X家企业，预计X亿
- 项目融资/固贷（CDMO/GMP/研发中心）：X家企业，预计X亿
- 研发贷/科技信贷（创新药研发/临床）：X家企业，预计X亿
- 订单融资/保理（集采中标/CXO大单）：X家企业，预计X亿
- 流贷追加：X家企业，预计X亿
- 专精特新贷/高企贷：X家企业，预计X亿
- 供应链金融（原料药/医药流通）：X家企业，预计X亿
- 汇率锁汇（出口型CXO/器械）：X家企业，预计X亿
...
- XX生物（绍兴）：X亿融资到账，专户设立+流贷储备
- XX CXO（杭州）：海外大订单落地，关注订单融资需求

⚠️ 预警X条

- 集采降价超预期→企业利润承压、还款能力下降
- 核心品种被调出医保目录→收入大幅下滑风险
- 临床试验失败→研发管线价值缩水
...
- 今日/本周重大会议/发布会/政策生效日（如CDE审评会议、医保谈判、医药行业大会）
- 从财联社事件日历中提取生物医药相关事项
```
...
5. **知识库标题**：biomed-weekly-YYYYMMDD
...
**⚠️ 以下为当前 agent（SWYY Researcher）的参数。其他 agent 复用时需要替换为自己的值。**
