...
- **web_search 优先**：用关键词精准搜索智能制造行业信息，比爬取大站再筛选高效得多
...
```
1. "工业机器人" OR "协作机器人" OR "人形机器人" OR "AGV" OR "AMR" → web_search（近7天，count=10）
2. "数控机床" OR "CNC" OR "加工中心" OR "工业母机" OR "五轴联动" → web_search（近7天，count=10）
3. "工业软件" OR "MES" OR "PLM" OR "SCADA" OR "工业互联网" OR "数字孪生" → web_search（近7天，count=10）
4. "机器视觉" OR "智能质检" OR "AI+制造" OR "预测性维护" OR "边缘计算" → web_search（近7天，count=10）
5. "智能制造融资" OR "智能制造上市" OR "智能制造政策" OR "灯塔工厂" OR "智能工厂" → web_search（近7天，count=10）
```

**第三轮：智能制造垂直源补充（并行，4个）：**

```
6. 高工机器人网 → web_fetch https://www.gg-robot.com/ (extractMode="text", maxChars=6000)
7. 工控网（gongkong） → web_fetch https://www.gongkong.com/ (extractMode="text", maxChars=6000)
8. 智能制造网 / 智能制造产业联盟 → web_fetch https://www.zhizaojia.cn/ (extractMode="text", maxChars=6000)
9. 3D打印世界 / 增材制造 → web_fetch https://www.3dprint.cn/ (extractMode="text", maxChars=6000)
```

**第四轮：本地智能制造专项搜索（1个）：**

```
10. "智能制造 [地域名]" OR "工业机器人 [核心城市]" OR "数控机床 [另一城市]" OR "工业互联网 [地域名]" → web_search（近7天，count=10）
```
...
- 政策发布/修订（影响资金流向或授信风险）
- 企业融资/上市/定增公告
- 产能建设/项目开工/产线投产/新工厂奠基
- 订单/中标/框架协议公告
- 技术突破/产品发布（新机器人型号、五轴机床、工业软件新版本、AI质检方案等）
- 价格变动（机器人本体/减速器/伺服电机/控制器、数控系统、工业软件授权费等，影响产业链成本）
- 风险信号（订单下修、减持、诉讼、产能过剩预警、芯片/核心部件供应风险等）

**排除标准：**
- 行业常识科普
- 长期趋势分析（无具体事件）
- 与智能制造行业（工业机器人、数控机床、工业软件、AI+制造、3D打印、智能物流等）无关的内容
- 纯股市行情分析（无产业事件支撑）
...
```
企业 | 事件 | 资金需求/业务机会 | 优先级
XX机器人（<城市>）| IPO受理 | 8亿募资专户 | P0
XX智能装备（<城市>）| 项目公告 | 项目融资 | P1
XX数控（<城市>）| 中标大单 | 订单融资 | P1
```
...
- 订单融资/保理：X家企业，预计X亿
- 流贷追加：X家企业，预计X亿
- 科技信贷/专精特新贷：X家企业，预计X亿
- 供应链金融：X家企业，预计X亿
- 汇率锁汇（出口型智能制造企业）：X家企业，预计X亿
```
...
```
📌 智能制造行业周报·<地域> | YYYY.MM.DD-MM.DD（近7天）
...
[杭州]
- XX公司签约X条工业机器人产线项目（X月X日）
- XX智能工厂项目获批（X月X日）

[宁波]
- XX企业获X亿融资用于数控机床扩产（X月X日）
...
- XX数控（绍兴）：15亿融资到账，专户设立+流贷储备
- XX自动化（杭州）：灯塔工厂项目落地，关注项目融资需求

⚠️ 预警X条

- 减速器/伺服电机进口依赖度高→供应链风险
- 工业软件国产化进程不及预期→企业IT投入收缩
...
**⚠️ 以下为当前 agent（IM Researcher）的参数。其他 agent 复用时需要替换为自己的值。**
