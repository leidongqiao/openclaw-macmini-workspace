...
description: |
 生成半导体行业研究周报（聚焦浙江），每周爬取多源行业研究内容，去重筛选、
...
| `WEEKLY_TITLE_PREFIX` | 周报文档标题前缀（如 `bdt-weekly`、`ne-weekly`） | 自定义 |
...
```json
{
  ...
  "weekly_title_prefix": "bdt-weekly"
}
```
...
**研究方向：半导体全产业链**（芯片设计、晶圆制造、封测、设备、材料、功率半导体、MEMS、车规芯片、第三代半导体等）

## Token 优化原则

- **web_search 优先**：用关键词精准搜索半导体行业信息，比爬取大站再筛选高效得多
...
```
1. "半导体" OR "芯片" OR "集成电路" → web_search（近7天，count=10）
2. "晶圆制造" OR "先进封装" OR "Chiplet" OR "HBM" → web_search（近7天，count=10）
3. "半导体设备" OR "光刻机" OR "刻蚀" OR "EDA" → web_search（近7天，count=10）
4. "功率半导体" OR "碳化硅" OR "氮化镓" OR "车规芯片" → web_search（近7天，count=10）
5. "半导体融资" OR "半导体上市" OR "半导体政策" OR "半导体投资" → web_search（近7天，count=10）
```

**第三轮：半导体垂直源补充（并行，3个）：**

```
6. 半导体行业观察 → web_fetch https://www.smi-China.com/  (extractMode="text", maxChars=6000)
7. 芯智讯 → web_fetch https://www.36kr.com/information/Chip/  (extractMode="text", maxChars=6000)
8. 电子发烧友 → web_fetch https://bbs.elecfans.com/  (extractMode="text", maxChars=6000)
```

**第四轮：本地半导体专项搜索（1个）：**

```
9. "半导体 [地域名]" OR "芯片 [核心城市]" OR "半导体 [另一城市]" OR "集成电路 [地域名]" → web_search（近7天，count=10）
```
...
- 订单/定点公告
- 技术突破/产品发布
- 价格变动（晶圆代工、存储芯片、功率器件、半导体材料等，影响产业链成本）
- 风险信号（订单下修、减持、诉讼等）

**排除标准：**
- 行业常识科普
- 长期趋势分析（无具体事件）
- 与半导体行业（芯片设计、晶圆制造、封测、半导体设备、材料、功率半导体、MEMS、车规芯片、第三代半导体等）无关的内容
- 纯股市行情分析（无产业事件支撑）
...
```
企业 | 事件 | 资金需求/业务机会 | 优先级
XX科技（<城市>）| IPO受理 | 8亿募资专户 | P0
XX集成（<城市>）| 产线公告 | 设备付汇 | P1
```
...
**🔴 关键规则（必须严格遵守）：**
- **只写入「必访」企业**：仅周报行动清单中「🔴 必访」栏目下的企业才写入表格，「储备」「预警」企业一律不写入
- **只写入目标区域本地企业**，非目标区域企业一律不写入表格
- **写入前必须去重**：先读取 A 列，用「规范化核心简称精确匹配」判断是否已存在
- **禁止重复写入同一企业**：同一核心简称只保留一行，已有行则更新，无则追加
- **非必访企业清理**：表格中已存在但本期不在「必访」名单中的企业，状态为非终态则标记为「已移出」（不删除，方便追溯）
...
**写入流程（严格执行）：**
1. 从周报行动清单中提取「🔴 必访」企业名单（含 P 级、企业名称、城市、事件、方案、金额等）
2. 读取 A 列全量数据
3. 对 A 列每个非空名称做规范化：去掉括号内容，得到**已有核心简称列表**
4. 对本次必访企业先去重：按核心简称合并同名企业，每个核心简称只保留一条
5. 先检查非必访清理：对 A 列中每个已有行，若其核心简称不在本期必访名单中 → 标记状态为「已移出」（仅非终态行）
6. 对每个必访企业：提取核心简称 → 精确匹配 → 匹配到则更新，未匹配则追加
7. ⚠️ **更新已有商机时，创建日期（I列）必须更新为当前日期（YYYY-MM-DD）**
8. ⚠️ **如果匹配到的已有行状态为终态（closed/已关闭/已落地），则跳过该行，不更新**
9. ⚠️ **新增商机的状态列统一填写「待联系」，不要写 active/open**
10. ⚠️ **更新已有商机时，状态列保持不变，不修改**
11. ⚠️ **写入完成后，必须对整个表格进行时间倒序重排 + 清理残留空行**（见下方「表格排序与清理」）
...
SPREADSHEET_TOKEN = "RpI5svn81hl9axtuaqUcwtAenBM"
SHEET_ID = "89c832"
APP_ID = "cli_a9719ed2abf55bc8"
APP_SECRET = subprocess.run(
    ['bash', '-c', 'jq -r \'.channels.feishu.accounts.bdt_bot.appSecret\' ~/.openclaw/openclaw.json'],
...
⚠️ **关键说明：**
- 使用 bdt_bot 的 tenant_access_token 直接调 Sheets API，不依赖 lark-cli 配置
...
```
📌 半导体行业周报·<地域> | YYYY.MM.DD-MM.DD（近7天）
...
- XX芯片（杭州）：获重大订单，关注产能扩张计划

⚠️ 预警X条

- 存储芯片价格波动→下游模组厂成本压力传导
- XX板块高位震荡→关注股权质押风险
...
7. **群聊推送**：推送概要 + 链接，不是全文
8. **商机挖掘表格**：只写「必访」企业（🔴 必访栏目），「储备」「预警」企业不写入表格。写入前必须去重，只写目标区域本地企业。更新已有商机时日期必须更新为当天。写入后必须按时间倒序重排并清理残留空行
