...
description: |
 生成半导体行业研究周报（双版本：Word详细报告 + 飞书wiki，聚焦浙江），每周爬取多源行业研究内容，去重筛选、
 转化为银行商机清单，推送至飞书群。每周五执行，商机/行动清单/企业动态仅限浙江本地。
---

# 半导体行业研究周报生成 Skill

每周直接爬取多源半导体行业研究内容，生成双版本周报（Word详细报告 + 飞书 wiki 知识库存档），推送至飞书群。
...
1. **直接爬取**：每周从多源爬取半导体行业研究内容（行业报告、市场分析、技术趋势、政策动态、企业公告）。
2. **地域聚焦**：**本地行业动态、行动清单、企业推荐仅限浙江本地**（杭州/宁波/温州/绍兴/嘉兴/湖州/金华/台州/丽水/衢州/舟山）；长三角异动仅作为补充参考。
3. **TOP5-8精选**：本周最具实质影响的 5-8 条要闻。
...
```
5. 半导体综合 → searxng_search "半导体 OR 芯片 OR 集成电路" -n 10 --format json --time-range week
6. 先进制程/封装 → searxng_search "晶圆制造 OR 先进封装 OR Chiplet OR HBM" -n 10 --format json --time-range week
7. 设备/材料/EDA → searxng_search "半导体设备 OR 光刻机 OR 刻蚀 OR EDA" -n 10 --format json --time-range week
8. 功率/第三代半导体 → searxng_search "功率半导体 OR 碳化硅 OR 氮化镓 OR 车规芯片" -n 10 --format json --time-range week
9. 资本/政策动态 → searxng_search "半导体融资 OR 半导体上市 OR 半导体政策 OR 半导体投资" -n 10 --format json --time-range week
```
...
10. 爱集微/集微网 → web_fetch https://www.ijiwei.com/  (extractMode="text", maxChars=6000)
    - 若 web_fetch 失败，回退：searxng_search "site:ijiwei.com OR site:jiwei.cn 半导体 OR 芯片 OR 集成电路" -n 10 --format json --time-range week
...
```
13. 浙江区域半导体专项 → searxng_search "半导体 浙江 OR 芯片 杭州 OR 半导体 宁波 OR 集成电路 浙江" -n 10 --format json --time-range week
```

**第五轮：iFinD 结构化数据补充（4个查询，编号14-17，时间限定近7天）：**
...
> **时间参数**：`time_start` = 7天前日期（YYYY-MM-DD），`time_end` = 今天日期（YYYY-MM-DD）
...
```
14. search_news（新闻语义检索，严格7天）→ call("news", "search_news", {
  "query": "半导体 芯片 集成电路 人工智能 算力",
  "time_start": "<7天前>",
  "time_end": "<今天>",
  "size": 10
...
15. search_notice（公告语义检索，严格7天）→ call("news", "search_notice", {
...
2. **写入文件:**将即将推送的群聊摘要内容写入 `reports/summary/BDT-summary.md`。
...
# 写入摘要
echo '<摘要内容>' > ~/.openclaw/workspace/workspace-BDTresearcher/reports/summary/BDT-summary.md
```
